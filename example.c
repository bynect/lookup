...
main(int argc, char **argv)
{
    ...
    for (int i = 0; i < argc; ++i)
    {
        ...
        lookup_table_set(&t, keys + i, i + 100);
        lookup_table_set(&t2, keys + i, i + 200);
    }
    ...
    {
        intptr_t v1, v2;
        lookup_table_get(&t, keys + i, &v1);
        lookup_table_get(&t2, keys + i, &v2);
        printf("%s == %ld && %ld\n", argv[i], v1, v2);
    }
    ...
    {
        intptr_t val;
        lookup_table_get(&t3, keys + i, &val);
        printf("%s == %ld\n", argv[i], val);
    }
}
