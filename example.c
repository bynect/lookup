...
{
    lookup_table_t t1, t2, t3;
    lookup_table_init(&t1);
    lookup_table_init(&t2);
    lookup_table_init(&t3);
    ...
    for (int i = 0; i < argc; ++i)
    {
        ...
        lookup_table_insert(&t1, keys + i, i + 100);
        lookup_table_insert(&t2, keys + i, i + 200);
    }

    lookup_table_merge(&t3, &t1);
    lookup_table_merge(&t3, &t2);

    for (int i = 0; i < argc; ++i)
    {
        intptr_t v1, v2;
        lookup_table_find(&t1, keys + i, &v1);
